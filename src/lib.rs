...
pub fn parse<R: Read, H: parse::Handlers>(
    r: &mut R,
    handlers: &mut H,
    opts: Option<parse::Opts>,
) -> std::result::Result<(), ParseError> {
    let mut r = TrackingReader { pos: 0, reader: r };
    parse::parse(&mut r, handlers, opts).map_err(|e| ParseError {
        error: e,
        pos: Some(r.pos),
    })
}
...
pub fn game<R: Read>(r: &mut R, opts: Option<parse::Opts>) -> Result<game::Game, ParseError> {
    ...
    parse(r, &mut game_parser, opts).and_then(|_| {
