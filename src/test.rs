...
use super::{
    ...
    frame::{self, Buttons},
    game::{DashBack, End, EndMethod, Frames, Game, Player, PlayerType, ShieldDrop, Start, Ucf},
    item::Item,
...
fn game(name: &str) -> Result<Game, String> {
    let mut buf =
        io::BufReader::new(fs::File::open(&format!("test/replays/{}.slp", name)).unwrap());
