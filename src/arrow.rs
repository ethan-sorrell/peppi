...
use arrow::{
    ...
    buffer,
    datatypes::{
        self, BooleanType, DataType, Field, Float32Type, Int16Type, Int32Type, Int64Type, Int8Type,
    ...
    T: ArrowPrimitive,
{
    fn arrow_buffers(path: &Vec<String>, len: usize, _slippi: Slippi, _opts: Opts) -> Vec<Buffer> {
        vec![Buffer::new(
            path,
            len * mem::size_of::<T::ArrowNativeType>(),
            T::ARROW_DATA_TYPE,
        )]
    }
    ...
    fn arrow_append_null(
        ...
        buffers[index]
            .arrow_buffer
            .extend_zeros(mem::size_of::<T::ArrowNativeType>());
        1
    }
}
    ...
    T: Arrow,
{
    fn arrow_buffers(path: &Vec<String>, len: usize, slippi: Slippi, opts: Opts) -> Vec<Buffer> {
        ...
        for i in 0..N {
            ...
            let bufs = T::arrow_buffers(&clone_push(path, name.clone()), len, slippi, opts);
...
impl Arrow for frame::PortData {
    fn arrow_buffers(path: &Vec<String>, len: usize, slippi: Slippi, opts: Opts) -> Vec<Buffer> {
        ...
        if true {
            let bufs = frame::Data::arrow_buffers(&clone_push(path, "leader"), len, slippi, opts);
    ...
    fn arrow_append(
        ...
        offset += self
            .leader
            .arrow_append(buffers, index + offset, slippi, opts);
...
fn _frames(frames: &Vec<frame::Frame>, slippi: Slippi, opts: Opts) -> ArrayRef {
    let mut buffers = frame::Frame::arrow_buffers(&vec![String::new()], frames.len(), slippi, opts);
    for frame in frames {
...
pub fn frames(game: &game::Game, opts: Option<Opts>) -> ArrayRef {
    ...
    _frames(&game.frames, slippi, opts)
}

fn _items(frames: &Vec<frame::Frame>, slippi: Slippi, opts: Opts) -> Option<ArrayRef> {
    if slippi.version >= Version(3, 0, 0) {
...
pub fn items(game: &game::Game, opts: Option<Opts>) -> Option<ArrayRef> {
    ...
    let slippi = game.start.slippi;
    _items(&game.frames, slippi, opts)
}
